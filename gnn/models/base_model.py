...
entity_embedding（知識圖譜實體）

relation_embedding（關係）

提供損失函數（KL、BCE）

提供 Hit@1、F1 評估

特色：不做推理，只提供「資源 (embedding, loss, metrics)」。
...
class BaseModel(torch.nn.Module):
    ...
    def embedding_def(self):
        ...
        #外部詞向量檔 (.npy)，就載入 → 例如 Glove / word2vec 才會需要用到但這不會用因為醫學文本要用
        ...
        #我應該是用到這個
        #實體向量大小完全由超參數決定 例如768
        else:
            self.ent_dim = self.kg_dim 
            self.encode_type = True     # 因為我這裡建立了 所以 ReaRev.py的 if self.encode_type: 
                                        #  self.type_layer = TypeLayer( 會被觸發
    ...
    def encode_rel_texts(self, rel_texts, rel_texts_inv):
        """
        Encode relation texts (正向 & 反向)，得到 entity_dim 的向量
        """
        self.rel_texts = torch.from_numpy(rel_texts).type('torch.LongTensor').to(self.device)
        self.rel_texts_inv = torch.from_numpy(rel_texts_inv).type('torch.LongTensor').to(self.device)
        ...
        with torch.no_grad():
            # === 正向關係 ===
            texts = [self.instruction.tokenizer.decode(t, skip_special_tokens=True) for t in rel_texts]
            inputs = self.instruction.tokenizer(texts, return_tensors="pt", padding=True, truncation=True)
            inputs = {k: v.to(self.device) for k, v in inputs.items()}
            self.rel_features = self.instruction.encode_question(inputs, store=False)  # 直接回 CLS -> entity_dim

            # === 反向關係 ===
            texts_inv = [self.instruction.tokenizer.decode(t, skip_special_tokens=True) for t in rel_texts_inv]
            inputs_inv = self.instruction.tokenizer(texts_inv, return_tensors="pt", padding=True, truncation=True)
            inputs_inv = {k: v.to(self.device) for k, v in inputs_inv.items()}
            self.rel_features_inv = self.instruction.encode_question(inputs_inv, store=False)
