...
class BasicDataLoader(object):
    ...
    def _extract_edges_from_tuples(self, sample, g2l, sample_id):
        ...
        # === Debug: 打印原始 tuples 數量和 fact 總數 ===
        print(f"[DEBUG] sample_id={sample_id} | tuples={len(sample['subgraph']['tuples'])} | facts={len(head_list)}")
  ...
  # 保證 index 對齊
    ...
    def _build_fact_mat(self, sample_ids, fact_dropout):
        ...
        # === Debug ===
        print(f"[DEBUG] build_fact_mat | sample_ids={sample_ids} | expected={expected_facts} | final={len(batch_heads)}")
