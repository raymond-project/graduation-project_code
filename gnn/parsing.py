...
def add_shared_args(parser):
    ...
    parser.add_argument('--data_folder', type=str)   #不給預設
    ...
    parser.add_argument('--lm_frozen', type=int) #微調語言模型（讓 BERT 一起學）
...
def create_parser_rearev(parser):
    ...
    parser.add_argument('--use_self_loop', action='store_true', help="是否使用 self loop")
...
def create_parser_nsm(parser):
    ...
    parser.add_argument('--use_inverse_relation', action='store_true', default=True)
    parser.add_argument('--norm_rel', action='store_true')
    parser.add_argument('--normalized_gnn', default=False, type=bool_flag)
    parser.add_argument('--data_eff', action='store_true')
    add_shared_args(parser)
