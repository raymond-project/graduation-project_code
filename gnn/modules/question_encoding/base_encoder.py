...
class BaseInstruction(torch.nn.Module):
    ...
    def __init__(self, args, constraint):
        super(BaseInstruction, self).__init__()   # PyTorch nn.Module.__call__ 會觸發 forward 所以實際上有觸發 你在
        #self.instruction_list, self.attn_list = self.instruction(q_input)  的rearav.py使用這都時候觸發forward的
    ...
    def init_reason(self, query_text):
        ...
        query_strings = [self.tokenizer.decode(q, skip_special_tokens=True) for q in query_text]
        query_input = self.tokenizer(query_strings, return_tensors="pt", padding=True, truncation=True)
        query_input = {k: v.to(query_text.device) for k, v in query_input.items()}
...
# 回傳給 self.instruction_list, self.attn_list = self.instruction(q_input) 
#給rearav.py  給他以下self.instructions, self.attn_list 
""" 
self.instruction_list (instructions)
list/sequence，長度 = num_ins
每個元素是一個 query 向量 (entity_dim 維度)
代表「每一步 reasoning 的 query 表徵」
可以想成「問題被分解成多步指令」

==============================================

self.attn_list (attention weights)
list/sequence，長度 = num_ins
每個元素是 (batch, seq_len, 1) 的 attention 分佈
代表在第 i 步，模型對 query 的 token-level 注意力
